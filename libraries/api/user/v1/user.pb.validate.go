...
func (m *UserInfo) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetName()) > 25 {
...
func (m *UpdateRequest) validate(all bool) error {
	...
	if wrapper := m.GetName(); wrapper != nil {
		...
		if utf8.RuneCountInString(wrapper.GetValue()) > 25 {
...
// Validate checks the field values on GetListFilters with the rules defined in
// the proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *GetListFilters) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on GetListFilters with the rules defined
// in the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in GetListFiltersMultiError,
// or nil if none found.
func (m *GetListFilters) ValidateAll() error {
	return m.validate(true)
}

func (m *GetListFilters) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	_GetListFilters_Names_Unique := make(map[string]struct{}, len(m.GetNames()))

	for idx, item := range m.GetNames() {
		_, _ = idx, item

		if _, exists := _GetListFilters_Names_Unique[item]; exists {
			err := GetListFiltersValidationError{
			...
			_GetListFilters_Names_Unique[item] = struct{}{}
		}

		if utf8.RuneCountInString(item) > 25 {
			err := GetListFiltersValidationError{
				field:  fmt.Sprintf("Names[%v]", idx),
				reason: "value length must be at most 25 runes",
			}
	...
	if len(errors) > 0 {
		return GetListFiltersMultiError(errors)
	}

	return nil
}

// GetListFiltersMultiError is an error wrapping multiple validation errors
// returned by GetListFilters.ValidateAll() if the designated constraints
// aren't met.
type GetListFiltersMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m GetListFiltersMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m GetListFiltersMultiError) AllErrors() []error { return m }

// GetListFiltersValidationError is the validation error returned by
// GetListFilters.Validate if the designated constraints aren't met.
type GetListFiltersValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e GetListFiltersValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e GetListFiltersValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e GetListFiltersValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e GetListFiltersValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e GetListFiltersValidationError) ErrorName() string { return "GetListFiltersValidationError" }

// Error satisfies the builtin error interface
func (e GetListFiltersValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = GetListFiltersValidationError{}
