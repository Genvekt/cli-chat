...
func TestGetRefreshToken(t *testing.T) {
	type args struct {
		ctx context.Context
		req *authApi.GetRefreshTokenRequest
	}
	...
	var (
		...
		res = &authApi.GetRefreshTokenResponse{
	...
	tests := []struct {
		name                string
		args                args
		want                *authApi.GetRefreshTokenResponse
		err                 error
	...
	}{
		{
			...
			authServiceMockFunc: func(mc minimock.MockController) *serviceMock.AuthServiceMock {
				...
				mock.GetRefreshTokenMock.Expect(ctx, oldToken).Return(newToken, nil)
				return mock
			},
		},
		{
			...
			authServiceMockFunc: func(mc minimock.MockController) *serviceMock.AuthServiceMock {
				mock := serviceMock.NewAuthServiceMock(mc)
				mock.GetRefreshTokenMock.Expect(ctx, oldToken).Return("", serviceErr)
				return mock
			},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			apiRes, err := api.GetRefreshToken(tt.args.ctx, tt.args.req)
